...
```json
{
  ...
  "mcp": {
    "mcp_add": {
      "type": "local",
      "command": ["node", "/absolute/path/to/mcp_tutorial/server.js"],
      "enabled": true
    }
  }
}
...
**주의:**
- `command`는 **배열**로 지정 (문자열 아님)
- `enabled`는 서버 객체 **안**에 넣어야 함
- JSONC 형식이라 주석 사용 가능

### 2. 서버 재시작

OpenCode 재시작 후 MCP server 자동 로드됨.

### 3. 사용

OpenCode에서 직접 tool 호출:
```
Use the mcp_add tool with arguments: a=5, b=3
```
